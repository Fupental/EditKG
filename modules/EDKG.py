...
class SelectAgent(nn.Module):#用来定义primary KG和potential KG的两个APL
    """
    定义APL（Attribute Purification Layer，属性净化层）
...
class GraphConv(nn.Module):
    ...
    def forward(self, all_embed, all_embed_cf, edge_index, edge_type, n_edge_index, n_edge_type, 
        ...
        返回:
            ...
            mmd_loss: MMD蒸馏损失（推理时忽略）
        
        计算流程：
            1. 节点dropout（可选）
            2. 构建用户-物品和物品-用户稀疏矩阵
            3. 拆分all_embed为三组嵌入：CF嵌入、Primary KG嵌入、Potential KG嵌入
            4. 对两个KG分别执行APL筛选（Dnoise_KG）+ LLM掩码（如有）
            5. 多层KG消息传递 + 残差连接
            6. 拼接三组嵌入
            7. 多层CF聚合 + 残差连接
        """
...
class Recommender(nn.Module):
    ...
    def __init__(self, data_config, args_config, graph, ui_sp_graph, item_rel_mask, triplets=None, v_feat=None, t_feat=None):
        ...
        if v_feat is not None:
            self.v_feat = nn.Parameter(torch.FloatTensor(v_feat))  # [n_items, 384] 可训练
            print(f"[MM-DEBUG] v_feat 设为可训练参数, shape={self.v_feat.shape}, "
        ...
        if t_feat is not None:
            self.t_feat = nn.Parameter(torch.FloatTensor(t_feat))  # [n_items, 384] 可训练
            print(f"[MM-DEBUG] t_feat 设为可训练参数, shape={self.t_feat.shape}, "
...
# [已删除] KGC类 → 功能已被 LLMScorer (modules/llm_scorer.py) 完全替代
